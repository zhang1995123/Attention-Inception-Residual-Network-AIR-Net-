...
class Inception(nn.Module):
    ...
    def __init__(self, num_classes=10, inception_blocks=None):
        ...
        self.Conv2d_1a_3x3 = BasicConv2d(3, 16, kernel_size=3, stride=2, padding=1)

        self.Conv2d_2a_3x3 = BasicConv2d(16, 32, kernel_size=3, stride=1, padding=1)

        self.Conv2d_3a_1x1 = BasicConv2d(32, 64, kernel_size=1, padding=0)
        self.Conv2d_3b_3x3 = BasicConv2d(64, 128, kernel_size=3, padding=1)
...
class InceptionA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=32, conv_block=None):
        ...
        self.sample = BasicConv2d(in_channels, out_channels*4, kernel_size=1)
        self.se = SELayer_2(out_channels)

    def _forward(self, x):

        branch1x1 = self.branch1x1(x)
        branch1x1 = self.se(branch1x1)

        branch3x3 = self.branch3x3_1(x)
        branch3x3 = self.branch3x3_2(branch3x3)
        branch3x3 = self.se(branch3x3)

        branch5x5 = self.branch5x5_1(x)
        branch5x5 = self.branch5x5_2(branch5x5)
        branch5x5 = self.branch5x5_3(branch5x5)
        branch5x5 = self.se(branch5x5)

        branch_pool = F.avg_pool2d(x, kernel_size=3, stride=1, padding=1)
        branch_pool = self.branch_pool(branch_pool)
        branch_pool = self.se(branch_pool)
...
class InceptionB(nn.Module):
    ...
    def __init__(self, in_channels, conv_block=None):
        ...
        self.branch_1 = BasicConv2d(in_channels, 256, kernel_size=1, stride=2)

        self.branch_3_1 = BasicConv2d(in_channels, 64, kernel_size=1)
        self.branch_3_2 = BasicConv2d(64, 128, kernel_size=3, padding=1, stride=2)
    ...
    def _forward(self, x):
        ...
        branch_pool = F.max_pool2d(branch_pool, kernel_size=3, padding=1, stride=2)
...
class InceptionAux(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv0 = SeparableConv2d(in_channels, 128, kernel_size=3, padding=1)
        self.conv1 = SeparableConv2d(128, 128, kernel_size=3, padding=1)
        self.fc = nn.Linear(128, 1)

    def forward(self, x):
        x = self.conv0(x)
        x = self.conv1(x)
        x = F.adaptive_avg_pool2d(x, (1,1,1))
        x = torch.flatten(x, 1)
        x = self.fc(x)
...
class InceptionOut(nn.Module):
    ...
    def __init__(self, in_channels, conv_block=None):
        ...
        self.group_conv = SeparableConv2d(in_channels, 128, kernel_size=3, padding=1)
        self.relu = nn.ReLU(inplace=True)

        self.sample = BasicConv2d(in_channels, 128, kernel_size=1)
        self.se = SELayer_2(128)
...
class SeparableConv2d(nn.Module):
    def __init__(self, inplanes, outplanes, kernel_size=3, stride=1, padding=1, dilation=1):
        ...
        self.conv = nn.Conv2d(inplanes, inplanes, kernel_size, stride, padding, dilation,
                               groups=32, bias=False)
        self.bn1 = nn.BatchNorm2d(inplanes)
        self.pointwise = nn.Conv2d(inplanes, outplanes, 1, 1, 0, 1, 1, bias=False)
